...
// 회원가입 API 연동과정
1. #signup button 클릭
2. #userID, #password, nickname 값 확인 (RegExp 확인)
3. RegExp가 확인되면 회원가입 API 요청
...
async function signup(event) {
  ...
  /* 2. #email, #password, nickname 값 확인 (RegExp 확인) */
  ...
  // RegExp 통과 못할 시 alert message
  if (!userIDRegExp.test(userID)) {
    return alert("아이디 형식: 영문자로 시작되는 영문 또는 숫자 6-20자리");
  }
  if(!passwordRegExp.test(password)) {
    return alert("비밀번호 형식: 8-16자리 문자, 숫자 조합");
  }
  if(!nicknameRegExp.test(nickname)) {
    return alert("닉네임 형식: 2-10자리 숫자, 한글 또는 영문");
  }
